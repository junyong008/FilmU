...
class HandDetectorImpl @Inject constructor(
    ...
    override suspend fun getExtendedFingerCount(bitmap: Bitmap): Int = withContext(defaultDispatcher) {
        ...
        for (landmark in landmarks) {
            ...
            if (isFingerExtended(landmark[HandLandmark.INDEX_FINGER_TIP], landmark[HandLandmark.INDEX_FINGER_PIP], landmark[HandLandmark.WRIST])) count++
            if (isFingerExtended(landmark[HandLandmark.MIDDLE_FINGER_TIP], landmark[HandLandmark.MIDDLE_FINGER_PIP], landmark[HandLandmark.WRIST])) count++
            if (isFingerExtended(landmark[HandLandmark.RING_FINGER_TIP], landmark[HandLandmark.RING_FINGER_PIP], landmark[HandLandmark.WRIST])) count++
            if (isFingerExtended(landmark[HandLandmark.PINKY_TIP], landmark[HandLandmark.PINKY_PIP], landmark[HandLandmark.WRIST])) count++
            if (isThumbExtended(landmark[HandLandmark.THUMB_TIP], landmark[HandLandmark.THUMB_IP], landmark[HandLandmark.THUMB_CMC], landmark[HandLandmark.INDEX_FINGER_MCP], landmark[HandLandmark.PINKY_MCP])) count++
        }
        return@withContext count
    }

    private fun isFingerExtended(tip: NormalizedLandmark, pip: NormalizedLandmark, wrist: NormalizedLandmark): Boolean {
        val tipToWrist = calculateDistance(tip, wrist)
        val pipToWrist = calculateDistance(pip, wrist)
        return tipToWrist > pipToWrist
    }

    private fun isThumbExtended(tip: NormalizedLandmark, ip: NormalizedLandmark, cmc: NormalizedLandmark, idxMcp: NormalizedLandmark, pkyMcp: NormalizedLandmark): Boolean {
    ...
    private fun isStraightAngle(A: NormalizedLandmark, B: NormalizedLandmark, C: NormalizedLandmark): Boolean {
